...
namespace LevelDesign
{
    public class LevelGenerator : MonoBehaviour
    {
        ...
        { 
            get
            {
                if (useCliff) { return bottomLeft.position.x + (topRight.position.x - bottomLeft.position.x) / 10f; }
                else { return bottomLeft.position.x; }
            }
        }
        ...
        public float MaxX
        {
            get
            {
                if (useCliff) { return topRight.position.x - (topRight.position.x - bottomLeft.position.x) / 10f; }
                else { return topRight.position.x; }
            }
        }
        public float CenterX { get => (MinX + MaxX) / 2f; }

        public float MinY { get => bottomLeft.position.y; }

        public float MaxY { get => topRight.position.y; }

        public float CenterY { get => (MinY + MaxY) / 2f; }

        public Vector2 MinXY { get => new Vector2(MinX, MinY); }

        public Vector2 MaxXY { get => new Vector2(MaxX, MaxY); }

        public Vector2 CenterXY { get => new Vector2(CenterX, CenterY); }

        public Vector2 MinMaxX { get => new Vector2(MinX, MaxX); }

        public Vector2 MinMaxY { get => new Vector2(MinY, MaxY); }

        public Vector2 SizeXY { get => new Vector2(MaxX - MinX, MaxY - MinY); }

        public Bounds Bound { get => new Bounds(CenterXY, SizeXY); }
        ...
        [Header("浮动障碍物")]
        public List<GameObject> floatObjects;

        [Header("间隙规格")]
        [Range(0f, 17.8f)]
        public float gapX = 3f;
        [Range(0f, 10f)]
        ...
        [Header("默认标签和层")]
        public string defaultTag = "Combine";
        ...
        private void GenerateAnchorObstacle(int sel, Transform parent)
        {
            for (int i = 0; i < retryMax; i++)
            {
                ...
                Vector2 center = new Vector2(anchorRight ? MaxX - 0.5f * bdSize.x : MinX + 0.5f * bdSize.x, Random.Range(MinY, MaxY));
                Bounds bounds = new Bounds(center, bdSize);

                // 检查是否超出边界
                // 检查是否处与障碍区域相交
                if (!InRegion(bounds) || IntersectWithObstacle(bounds)) { continue; }
        ...
        private void MergeBoundsTo(BoxCollider2D coll)
        {
            ...
            Vector2 center = new Vector2(0.5f * (minX + maxX), 0.5f * (minY + maxY)) + (Vector2)(transform.position - coll.transform.position);
